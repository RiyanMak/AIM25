...
# [Rest of the code remains unchanged, but the extract_features function is indented back into FacialFeatureExtractor]

class FacialFeatureExtractor:
    def __init__(self, history_size=30):
        self.key_features = [
            'smile_amplitude', 'smile_symmetry', 'facial_mobility', 'mask_face_score'
        ]
    ...
    def extract_features(self, image):
        ...
        features.update({'expression_range': 0, 'microexpressions': 0})
        ...
        try:
            ...
            smile_measure = mouth_width / (np.linalg.norm(points[self.cheeks[0]] - points[self.cheeks[1]]) + 1e-6)

            # Improve neutral face logic
            if not self.neutral_established and smile_measure < 0.1 and mouth_height / mouth_width < 0.1:
            ...
            # Use more facial landmarks for better estimation
            left_eye = points[33]
            right_eye = points[263]
            nose_tip = points[4]
            left_cheek = points[self.cheeks[0]]
            right_cheek = points[self.cheeks[1]]
            
            # Calculate horizontal eye vector for yaw estimation
            eye_vector = right_eye - left_eye
            yaw_angle = np.arctan2(eye_vector[1], eye_vector[0])
            
            # Calculate distance from nose to each eye for roll estimation
            nose_to_left_eye = np.linalg.norm(nose_tip - left_eye)
            nose_to_right_eye = np.linalg.norm(nose_tip - right_eye)
            roll_factor = abs(nose_to_left_eye - nose_to_right_eye) / max(nose_to_left_eye, nose_to_right_eye)
            
            # Calculate cheek distance ratio for additional yaw estimation
            cheek_distance_ratio = np.linalg.norm(left_cheek - nose_tip) / (np.linalg.norm(right_cheek - nose_tip) + 1e-6)
            is_face_turned = abs(cheek_distance_ratio - 1.0) > 0.15 or abs(np.degrees(yaw_angle)) > 15
            
            # Only measure smile asymmetry when the face is relatively frontal
            if not is_face_turned:
                # Enhanced smile asymmetry detection
                # Use 3D coordinates to better measure smile corner heights
                
                # First get more points around the mouth for better analysis
                left_smile_region = [
                ...
                # Calculate vertical distances (y-coordinate) of smile corners from mouth center
                # In image coordinates, smaller y values are higher (top of image)
                ...
                # normalized by the mouth width for scale invariance
                ...
                # Face is turned - don't attempt to measure smile asymmetry
                features['smile_symmetry'] = 0.0

            # Remaining feature extraction
            ...
            mouth_movement = mouth_height / (mouth_width + 1e-6)
            ...
            if self.neutral_established:
                ...
                neutral_mouth_ratio = np.linalg.norm(self.neutral_landmarks[self.mouth_vertical[0]] - self.neutral_landmarks[self.mouth_vertical[1]]) / \
                                    (np.linalg.norm(self.neutral_landmarks[self.mouth_corners[1]] - self.neutral_landmarks[self.mouth_corners[0]]) + 1e-6)
                brow_mobility = abs(brow_movement - neutral_brow) / (neutral_brow + 1e-6)
                cheek_mobility = abs(cheek_movement - neutral_cheek) / (neutral_cheek + 1e-6)
                mouth_mobility = abs(mouth_movement - neutral_mouth_ratio) / (neutral_mouth_ratio + 1e-6)
                raw_mobility = 0.3 * brow_mobility + 0.3 * cheek_mobility + 0.4 * mouth_mobility
            ...
            if all(len(self.feature_history[f]) > 5 for f in ['smile_amplitude', 'facial_mobility']):
                expression_ranges = {
                    f: np.std(self.feature_history[f]) / (np.mean(self.feature_history[f]) + 1e-6)
                    for f in ['smile_amplitude', 'facial_mobility']
                }
                features['expression_range'] = min(1.0, 10 * (expression_ranges['smile_amplitude'] * 0.5 + expression_ranges['facial_mobility'] * 0.5))

            features['mask_face_score'] = (
                features['smile_symmetry'] * 0.15 +
                (1.0 - min(1.0, features['smile_amplitude'] * 3)) * 0.25 +
                (1.0 - features['facial_mobility']) * 0.3 +
                (1.0 - features['expression_range']) * 0.3
            )

            if features['smile_amplitude'] > 0.2 or features['facial_mobility'] > 0.2:
                features['mask_face_score'] = min(features['mask_face_score'], 0.5)
